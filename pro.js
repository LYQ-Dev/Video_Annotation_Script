...
<div style="margin-bottom:12px;"><label style="display:block; font-size:13px; color:#5f6368; margin-bottom:6px;">模型名称</label><input id="cfg-model" type="text" style="width:100%; box-sizing:border-box; padding:10px 12px; border:1px solid #dadce0; border-radius:8px; font-size:14px; outline:none; transition:border-color 0.2s;" value="${CONFIG.model}"></div>
<div style="margin-bottom:12px;"><label style="display:block; font-size:13px; color:#5f6368; margin-bottom:6px;">系统提示词 (Prompt)</label><textarea id="cfg-prompt" rows="3" style="width:100%; box-sizing:border-box; padding:10px 12px; border:1px solid #dadce0; border-radius:8px; font-size:14px; outline:none; transition:border-color 0.2s; resize:vertical; min-height:60px;">${CONFIG.prompt}</textarea></div>
<button id="test-api" style="width:100%; height:40px; background:#34a853; color:white; border:none; border-radius:8px; font-weight:500; cursor:pointer; transition:0.2s; margin-bottom:8px;">🟢 测试 API 连通性</button>
<button id="cfg-save" style="width:100%; height:42px; background:#1967d2; color:white; border:none; border-radius:8px; font-weight:500; cursor:pointer; transition:background 0.2s;">💾 保存并关闭</button>`;
    ...
    [document.getElementById('cfg-key'), document.getElementById('cfg-url'), document.getElementById('cfg-model'), document.getElementById('cfg-prompt')].forEach(inp => {
        ...
        try {
            ...
            const res = await new Promise((resolve, reject) => {
                ...
                GM_xmlhttpRequest({
                    ...
                    data: postData,
                    onload: r => r.status >= 200 && r.status < 300 ? resolve(r) : reject('状态码：' + r.status),
                    onerror: () => reject('网络错误')
                });
            });
